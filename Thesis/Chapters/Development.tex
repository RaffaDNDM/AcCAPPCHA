...
\begin{algorithm}[h]
\DontPrintSemicolon
\KwIn {$\mathtt{x =\{x_1,x_2,...,x_{|password|-1}\}}$= time instants stored by first thread\newline
$\mathtt{t =\{t_1,t_2,...,t_n\}}$= time instants relative to peaks of each group}
...
\BlankLine
 ...
 \emph{//already verified element in i}
 $\mathtt{j\gets} i+1$\;
 $\mathtt{count\gets} 1$\;
 \While{$count < |password| \wedge j < n$}{
  \If{$(n-i)<(|password|-count)$}{
   \BlankLine
   \Return $false$\;
   \BlankLine
  }
  \While{$j<n \wedge (t_j-t_i-0.5ms)<x_{count}$}
