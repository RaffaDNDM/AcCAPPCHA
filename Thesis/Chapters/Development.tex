...
\BlankLine
 ...
 \While{$count < |password| \wedge j < n$}{
  ...
  \uIf{$\mathtt{(t_j-t_i)}<(y_{count}-threshold)$}{
   	\emph{//Too less time between the time instant of the first character and the time instant of the \textbf{count}-th character}
    \BlankLine    
    $\mathtt{j}\gets j+1$\;
    \BlankLine
  }
  ...
  \Else{
    \emph{//Too much time between the time instant of the first character and the time instant of the \textbf{count}-th character}\;
    \BlankLine
    $\mathtt{break}$ \emph{//Try another time instant of next peak as first character inserted time instant}\;
    \BlankLine
  }
  \BlankLine
